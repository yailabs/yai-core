...
SHARED_BIN="${REPO_ROOT}/docs/labs/_shared/bin"
cd "$LAB_DIR"
...
if [ -n "$prompt_id" ]; then
  ...
  if [ "$prompt_kind" != "provider-chat" ]; then
    ...
    display_command="docs/labs/_shared/bin/probe-local-provider.sh --host \${YAI_PROVIDER_HOST:-127.0.0.1} --port \${YAI_PROVIDER_PORT:-43117} --prompt-id ${prompt_id}"
    set +e
    (
      cd "$REPO_ROOT"
