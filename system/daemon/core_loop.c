...
yai_status_t yai_daemon_run_filesystem_loop(const yai_daemon_ipc_request_t *request,
    ...
    if (append_record(&journal_file, &case_ref, 0, YAI_RECORD_CASE, "rec:new12-fs-case", 0, 0, 0, "case:opened daemon filesystem loop") != YAI_OK ||
        ...
        append_record(&journal_file, &case_ref, &policy_subject_ref, YAI_RECORD_POLICY_RULE, "rec:new12-fs-policy-rule-terminal", 0, 0, 0, "policy:manual-linenoise-terminal-interface-v0 rule:terminal-interface-no-authority effect:allow_observed constraint:no_decision_authority") != YAI_OK ||
