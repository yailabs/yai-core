...
| `work/archive/engineering-snapshots/` | Active implementation, command, source-surface, roadmap, testing, data-plane and pack docs. | mixed | move | Essential for maintainers, too wave-heavy for public portal. Extract public command/test/backend docs; move wave planning internal. | Public extracts to `docs/commands.md`, `docs/daemon.md`, `docs/backends.md`, `docs/test-cases.md`; planning to `docs/internal/engineering/` |
| `work/spines/command-surface.md` | Most complete command inventory. | yes | rewrite | Strong source for command docs, but current format is roadmap-centric and includes manual-test internals. | `docs/commands.md` |
| `work/spines/source-surface.md` | Current source boundary: C system roots, Rust engine target, transitional shims. | yes | rewrite | Should become public backend/source-boundary doc with less wave language. | `docs/backends.md` or `docs/architecture.md` |
| `work/spines/testing.md` | Test and validation guidance. | yes | rewrite | Should become concise reproducible test-case documentation. | `docs/test-cases.md` |
| `work/archive/status/` | Implementation status and import map. | mixed | move | Valuable for maintainers; too much wave history for public first contact. | `docs/internal/status/` |
| `work/archive/status/implementation-status.md` | Long wave history and current status. | no | move | Should not be first-contact public documentation. | `docs/internal/status/implementation-status.md` |
| `work/archive/protocols/` | First protocol shapes for records, receipts, projection, policy, memory, etc. | mixed | keep | Useful advanced technical reference, but should not be in the short public path until indexed. | `work/archive/protocols/` with link from `docs/architecture.md` only when stable |
...
| `work/archive/product/` | Positioning and wedge notes. | no | archive | Product positioning is not part of the technical public facade and risks sounding like marketing. | `docs/internal/product/` or `work/archive/legacy-docs/product/` |
| `work/archive/adr/` | Decision records and compendium. | mixed | keep | Useful for serious readers, but not primary portal material. | `work/archive/adr/` linked from `docs/architecture.md` as advanced context |
| `work/archive/legacy-docs/` | Historical engineering waves, maps, superseded docs, placeholder roots. | no | keep | Correct place for old material; keep out of main reading order. | `work/archive/legacy-docs/` |
| `docs/internal/` | Internal inventory area. | no | keep | Correct home for readiness maps and non-public planning. | `docs/internal/` |
| `work/archive/reference/figures/brand/yai-transp.png` | Brand figure used by current README. | yes | keep | Static asset. README rewrite can decide whether to keep image. | `work/archive/reference/figures/brand/` |
...
| `packaging/` | Placeholder. | yes | archive | No packaging target exists. Avoid public placeholder unless packaging is part of outreach. | `work/archive/legacy-docs/packaging/` or keep hidden until real |
...
|---|---|---|
| `README.md` | Short portal: what YAI is, what it is not, build/test entrypoints, license posture pointer. | Current `README.md`, `work/spines/source-surface.md`, `work/spines/command-surface.md` |
...
| `CONTRIBUTING.md` | Contribution scope, non-production posture, checks, expectations. | `Makefile`, `tools/checks/`, `work/spines/testing.md` |
...
| `docs/technical-brief.md` | Compact technical overview without marketing copy. | `README.md`, `work/archive/architecture-snapshots/00-spine.md`, `work/archive/architecture-snapshots/01-terminology.md` |
| `docs/architecture.md` | Public architecture summary and boundaries. | `docs/architecture/README.md`, architecture series |
| `docs/commands.md` | Supported command surface. | `work/spines/command-surface.md`, `Makefile` |
| `docs/daemon.md` | `yaid`, sockets, foreground/status/info/shutdown flow. | `work/spines/command-surface.md`, `system/daemon/`, `cmd/yaid/` |
| `docs/backends.md` | Backend/provider/data-plane boundaries. No untested DS4 claims. | `work/spines/source-surface.md`, `engine/README.md`, `system/*/README.md` |
| `docs/test-cases.md` | Reproducible test cases and expected commands. | `tests/smoke/`, `work/spines/testing.md`, manual validation doc |
| `docs/legal.md` | Plain-language licensing posture and pointers, not final legal terms. | Current `LICENSE`; legal review required |
| `docs/glossary.md` | Terms such as case, subject, op, control, carrier, receipt, projection. | Current `README.md`, `work/archive/architecture-snapshots/01-terminology.md` |
...
| `work/archive/product/` | `docs/internal/product/` | move | Keep product notes out of public technical facade. |
| `work/archive/status/` | `docs/internal/status/` | move | Keep implementation history available but not first-contact public. |
| Long-form `work/archive/engineering-snapshots/*.md` wave/planning files | `docs/internal/engineering/` | move | Extract concise public docs first. Do not strand links. |
| `work/archive/legacy-docs/` | `work/archive/legacy-docs/` | keep | Already correctly separated. |
| `packaging/README.md` | `work/archive/legacy-docs/packaging/README.md` | archive | Only if packaging remains placeholder after REPO.0. |
...
docs/legal.md
docs/glossary.md
work/archive/protocols/ when linked as advanced reference
work/archive/adr/ when linked as advanced reference
```
...
- `work/archive/product/` exists inside the public docs tree. It should move internal or
...
work/archive/status/implementation-status.md
examples/README.md
tests/README.md
tools/README.md
vendor/README.md
...
work/archive/product/
work/archive/protocols/
