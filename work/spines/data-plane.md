...
`work/archive/legacy-docs/engineering/roadmaps/` and `work/archive/legacy-docs/engineering/superseded/`.
...
The matrix spans `labs/filesystem-loop` and `labs/filesystem-loop`:
...
SPINE.46 initializes schema `yai.fact.v1` at
`YAI_HOME/store/facts/yai-facts.duckdb` with `yai facts status`,
`yai facts schema` and `yai facts init`. The initial DuckDB tables are:
...
`work/spines/command-surface.md` maps the hot-state primitive, snapshot
...
inspection remains future visual doctrine in `labs/filesystem-loop`.
