...
Status: SPINE.5 DATA spine roadmap, refined by NEW.13 through NEW.19.
...
SPINE.4 operational observability/evaluation plane
NEW.18C interaction thread / participant view boundary
SPINE.5 core operational spine + work breakdown rebase
...
SPINE.5 status: done core operational spine + work breakdown rebase. DATA is
now one macro-spine inside the larger `WORLD`, `RESIDUE`, `DATA`, `VIEW`,
`CONTROL`, `MEMORY`, `RECONCILE`, `HOST`, `MODEL`, `OBSERVABILITY` and
`EXTERNAL` map.

NEW.19 status: done Makefile/build/guards realignment. NEW.20 local install
layout is next.
