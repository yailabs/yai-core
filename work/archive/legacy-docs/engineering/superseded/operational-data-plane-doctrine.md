...
Status: SPINE.5 DATA spine doctrine, based on SPINE.3R.
...
projection and reconcile. NEW.19 has aligned build and guard rules to that
target posture.
...
commands or rebuild tooling. It made the measurement surface canonical before
NEW.19 guard realignment.

## SPINE.5 Work Breakdown

SPINE.5 tracks this doctrine as the `DATA` macro-spine. Future data-plane waves
must declare which subdeliveries they mature, from hot state and journal replay
through LMDB records, Ladybug graph, DuckDB facts, projection deltas, memory
consolidation, reconcile consistency and data-plane rebuild tests.
