...
if ! grep -q 'SPINE.26 Hot State Projection Freshness Integration' "$ROOT/docs/engineering/current-status.md"; then
  printf 'current-status does not name SPINE.26 hot state projection freshness integration\n' >&2
  exit 1
fi
