...
| Concern | Path |
|---|---|
| Entry point and protocol | `labs/external-runtime/README.md` |
| Runbook | `labs/external-runtime/runbook.md` |
| Notebook | `labs/external-runtime/notebook.ipynb` |
| Prompt catalog | `labs/external-runtime/prompts.json` |
| Run script | `labs/external-runtime/run.sh` |
| Runs | `labs/external-runtime/runs/` |
...
`labs/shared/bin/probe-local-provider.sh`, and stores the request payload,
...
```bash
labs/external-runtime/run.sh --slug canon6-provider-probe --prompt-id local-provider-ready
```
...
```bash
labs/external-runtime/run.sh --slug <slug> --command "true"
```
...
```bash
python3 labs/shared/bin/generate-run-figures.py --run-dir labs/external-runtime/runs/<run> --update-report --overwrite
```
