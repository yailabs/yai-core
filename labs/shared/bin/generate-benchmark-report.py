...
def try_optional_endpoint_latency(path, endpoints):
    return False


def try_optional_status_matrix(path, endpoints):
    return False


def try_optional_hnsw(path, hnsw_rows):
    return False


def try_optional_topology_html(path, normalized):
    return False


def write_endpoint_latency_svg(path, endpoints):
    if try_optional_endpoint_latency(path, endpoints):
        return
...
def write_endpoint_status_matrix_svg(path, endpoints):
    if try_optional_status_matrix(path, endpoints):
        return
    ...
    body = [
        ...
        '<text x="90" y="72" font-family="Arial" font-size="13" fill="#4b5563">Reachability and authentication posture for curated local probes.</text>',
    ]
...
def write_hnsw_svg(path, hnsw_rows):
    if try_optional_hnsw(path, hnsw_rows):
        return
...
def write_topology_html(path, normalized):
    if try_optional_topology_html(path, normalized):
        return
    ...
    topology_description = (
        "Three-dimensional view of the local benchmark boundary. The live model discovery path is separated from the chat authentication gate and the refused port."
        if negative_ports
        else "Three-dimensional view of the local benchmark boundary. The live model discovery path is separated from the chat authentication gate."
    )
    payload = json.dumps(
        {
...
The curated local provider evidence shows that `127.0.0.1:{live_port}` is the live provider port for model discovery{negative_summary}. The provider returns `reachable` for `/v1/models` and `auth_blocked` for `/v1/chat/completions`; the current blocker is provider authentication, not local reachability.
