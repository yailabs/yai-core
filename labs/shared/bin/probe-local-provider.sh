...
host="${YAI_PROVIDER_HOST:-localhost}"
port="${YAI_PROVIDER_PORT:-43117}"
model="${YAI_PROVIDER_MODEL:-qwen-local}"
out_dir=""
prompt=""
max_tokens="${YAI_PROVIDER_MAX_TOKENS:-96}"
temperature="${YAI_PROVIDER_TEMPERATURE:-0}"
api_key="${YAI_PROVIDER_API_KEY:-${OPENCODE_LLM_API_KEY:-}}"
...
if [ -z "$out_dir" ]; then
  out_dir="benchmarks/external-runtime/local-provider-$(date +%Y%m%d-%H%M%S)"
fi

if [ -z "$prompt" ]; then
  printf 'missing required --prompt; resolve prompt text from a lab prompts.json catalog first\n' >&2
  exit 2
fi
...
python3 - "$out_dir/chat-request.json" "$model" "$prompt" "$max_tokens" "$temperature" <<'PY'
import json
import sys

path, model, prompt, max_tokens, temperature = sys.argv[1:6]
payload = {
    "model": model,
    "stream": False,
...
{
  "schema": "yai.local_provider_probe.v1",
...
printf 'local provider probe written: %s\n' "$out_dir"
